...
import {
  ...
  GUIDE_CONVERGE, GUIDE_BUDGET, GUIDE_DEEP, GUIDE_OFFPEAK, GUIDE_BUDGET_TIGHT,
} from './core.js?v=4' // v=4：ESM 按 URL 缓存，热重载须换 query（v4 起：自动成本路由）

/** 设置预算上限时向用户解释「收敛」与「预算上限」的关系（面板 note + 工具返回共用）。 */
const BUDGET_RELATION_NOTE = '预算与收敛的关系：预算=硬上限——设了预算**且该项目成本守卫开启**（cost_enable on / 面板「项目」开关），花到即由 llm/stream 短路拦截，停止进一步计费（到顶即停；在途这一发会走完）。守卫关闭时预算只记录、不拦截。到达前由自动成本路由的预算防线（动态 1−2σ/B）先触发收敛收尾。cost_budget 0 = 清除预算 = 无硬顶。建议配合首轮收敛（Router Flash + 成本守卫 预设或任务收敛后缀）——先收敛少花，撞线硬停。'
...
const OPTAB_FILE = join(BASE, 'optab.json')
...
/** dsh-optab 托管开关（会话级）：~/.dsh/cost-guard/optab.json = { enabled: 全局默认, sessions: {会话id:bool} } */
function loadOptab() {
  try { return { enabled: false, sessions: {}, ...(existsSync(OPTAB_FILE) ? JSON.parse(readFileSync(OPTAB_FILE, 'utf8')) : {}) } }
  catch { return { enabled: false, sessions: {} } }
}
function saveOptab(state) {
  try {
    const cur = loadOptab()
    if (state.sessionId) {
      // 会话级覆盖：只写该会话，不动全局默认
      if (typeof state.enabled === 'boolean') cur.sessions[state.sessionId] = state.enabled
    } else if (typeof state.enabled === 'boolean') {
      cur.enabled = state.enabled
    }
    writeFileSync(OPTAB_FILE, JSON.stringify({ enabled: Boolean(cur.enabled), sessions: cur.sessions, updatedAt: new Date().toISOString() }, null, 2), 'utf8'); return true
  } catch { return false }
}
...
export function apply(ctx) {
  ...
  const pendingUserText = new Map() // session id -> 尚未被 firstCapture 消费的首条用户文本（headless 事件序兼容）
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    if (agent !== undefined && agent.session !== undefined) {
      ...
      if (firstCapture) {
        maybeAutoRoute(agent.session, null, pendingUserText.get(agent.session.id))
        pendingUserText.delete(agent.session.id)
      }
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (event.type === 'user/message' && event.data?.source?.kind === 'user') {
      const c = event.data?.message?.content || event.data?.content
      if (Array.isArray(c)) {
        const text = c.filter((b) => b.type === 'text').map((b) => b.text).join('')
        if (text) pendingUserText.set(session.id, text)
      }
      if (enabledForSession(session)) checkBudget(session)
      maybeAutoRoute(session, event)
    }
  })
  ...
  // ── 自动成本路由（原「自动深度分流」升级为三维自动配合）──────────────────
  // config.json 里 config.autoDeep.enabled=true 开启；命中复杂度关键词的消息不再
  // 固定注入「深度」引导，而是按 复杂度 × 峰谷 × 预算余量 自动选档：
  //   ① 预算已超（spent ≥ budget）→ 不叠加（checkBudget 已注入预算止损引导）
  //   ② 预算余量紧张（spent/budget ≥ budgetGuardRatio）→ 注入「收敛」（深度让位，防在超支边缘多花钱）
  //   ③ 高峰时段（×2 价）→ 注入「错峰」（可延时的重活登记 cost_defer 空闲自动跑；必须现在做则收敛）
  //   ④ 空闲 + 预算充足 → 注入「深度」（做对避免返工 = 避免重复计费）
  //  手动收敛模式永远优先 → 完全不介入（尊重用户，避免与收敛指令矛盾）
  // 近场注入一律 force=true：自动成本路由与「收敛」按钮一样随时可用（不依赖守卫开关）。
  const DEEP_KEYWORDS = ['重构', '架构', '设计方案', '系统设计', '评审', '迁移', '集成', '性能', '复杂', '大规模', '分布式', '微服务', '并发', '安全', '数据库', '算法', '从零构建', '框架', '长期', '拆解', '兼容', '边界情况', '单元测试', '设计文档', '方案']
  const routedSessionId = new Set() // session id -> 已注入过自动成本路由引导（每会话一次，避免刷屏）
  function lastUserText(session) {
    ...
    for (let i = events.length - 1; i >= 0; i--) {
      ...
      const c = ev.data?.message?.content || ev.data?.content
  ...
  // 收敛防线的动态阈值（量化版分层兜底）：
  //   固定数字 = 显式覆盖（兼容）；缺省或 "auto" = 动态自适应
  //     T = clamp(1 − 2·σ_last / B, 0.3, 0.95)
  //   σ_last = 上一轮用户交互的完整成本（lastTurnOf × 当前费率，含峰谷×2），
  //   随会话上下文增长自动上升、高峰自动翻倍 → 阈值自动提前；B 越小越早收敛。
  //   物理含义：留出 ~2σ 跑道（一轮在途拦不住 + 收敛收尾），让收敛后还能在闸门前交付。
  function effectiveGuardRatio(cfg, session, eff) {
    const raw = cfg?.budgetGuardRatio
    if (typeof raw === 'number' && Number.isFinite(raw)) return Math.max(0.05, Math.min(1, raw))
    const B = eff?.budget
    if (typeof B !== 'number' || !Number.isFinite(B) || B <= 0) return 0.8
    const lt = lastTurnOf(session?.events)
    const sigma = lt ? costOf(lt.totals, ratesFor(modelOf(session))).yuan : 0
    if (!(sigma > 0)) return 0.8
    return Math.max(0.3, Math.min(0.95, 1 - (2 * sigma) / B))
  }
  /** cost_status 里展示当前项目生效的防线阈值（含推导过程）。 */
  function guardThresholdInfo(session, eff) {
    if (eff?.budget === undefined || eff?.budget <= 0) return '—（未设预算）'
    const B = eff.budget
    const raw = config.autoDeep?.budgetGuardRatio
    if (typeof raw === 'number' && Number.isFinite(raw)) return `fixed=${raw}`
    const lt = lastTurnOf(session?.events)
    const sigma = lt ? costOf(lt.totals, ratesFor(modelOf(session))).yuan : 0
    if (!(sigma > 0)) return 'auto=0.800（暂无σ样本）'
    return `auto=${(Math.max(0.3, Math.min(0.95, 1 - (2 * sigma) / B))).toFixed(3)}  (σ_last=¥${sigma.toFixed(3)} → 1−2σ/B)`
  }
  function maybeAutoRoute(session, event, fallbackText) {
    const cfg = config.autoDeep
    if (!cfg || !cfg.enabled) return
    if (routedSessionId.has(session.id)) return
    let text = lastUserText(session)
    if (!text && (event?.data?.message?.content || event?.data?.content)) {
      const c = event.data.message?.content || event.data.content
    ...
    // 手动收敛模式 → 尊重用户，不介入（含所有子分支）
    if (lastConvergeMsg.has(session.id) || convergedSessions.has(session.id)) return
    const eff = budgetOf(session)
    const yuan = costOf(sessionTotals(session), ratesFor(modelOf(session))).yuan
    // ① 已超支：checkBudget 在同一事件里已注入止损，这里不再叠加
    if (eff.budget !== undefined && eff.budget > 0 && yuan >= eff.budget) return
    // ②③④ 决策：预算防线优先于峰谷；峰谷优先于默认深度。
    // budgetGuardRatio：数字 = 固定阈值（保留兼容）；缺省或 "auto" = 动态阈值
    //   T = clamp(1 − 2·σ_last / B, 0.3, 0.95)，σ_last = 上一轮完整交互成本（含峰价）。
    let kind = 'deep'
    let thr = null
    if (eff.budget !== undefined && eff.budget > 0) {
      thr = effectiveGuardRatio(cfg, session, eff)
      if (yuan / eff.budget >= thr) kind = 'converge'
    }
    if (kind === 'deep' && isPeakHour()) kind = 'offpeak'
    const guide = { converge: GUIDE_BUDGET_TIGHT, offpeak: GUIDE_OFFPEAK, deep: GUIDE_DEEP }[kind]
    // headless/session 事件内直接 append 会触发 "cannot reenter while another append is being published"，
    // 这里统一放到下一事件循环再注入；routedSessionId 先占位防止重复调度。
    routedSessionId.add(session.id)
    setImmediate(() => {
      const msgId = injectGuide(session, guide, true)
      if (msgId !== null) {
        log({ event: 'auto-route', session: session.id.slice(0, 8), project: projectOfId(session.id),
          kind, keyword: hit, chars: text.length, thr,
          ratio: eff.budget !== undefined && eff.budget > 0 ? Number((yuan / eff.budget).toFixed(3)) : null })
      }
    })
  }
  ...
  // ── 预算 = 硬上限：llm/stream waterfall 短路，到顶后不再产生任何计费调用 ──
  // 预算不再是旧的"软性超支提醒"，而是直接作为硬顶：设了预算（项目级/会话级）**且该项目
  // 成本守卫开启**时才生效（项目/全局开关关着时预算只统计、不拦截）。agent-loop 的交互请求
  // 都带 sessionId（loop 盖章）；当该会话的有效预算已花 ≥ 预算时，本中间件短路返回固定终止流
  // （不调用 provider = 不计费）。非 loop 自建流（错峰 daemon 等）没有 sessionId → 一律放行。
  // 正在飞行的那一次请求会走完（无法半途退费），但此后的新请求全部短路。清除预算
  // （cost_budget 0）= 回到无硬顶；守卫关闭 = 预算不生效。
  /** 命中预算硬顶返回给模型的终止文本；未命中返回 null。 */
  function gateDecision(sid) {
    if (typeof sid !== 'string' || !sid) return null
    const project = projectOfId(sid)
    // 预算效果以「成本守卫开启」为前提：项目/全局开关关着时，预算只统计、不拦截
    if (!enabledForProject(project)) return null
    const agent = agents.get(sid)
    const session = agent?.session
    const pb = budgetForProject(project)
    const eff = session ? budgetOf(session)
      : (pb !== undefined ? { budget: pb, scope: 'project', project } : { budget: undefined, scope: 'none', project })
    if (eff.budget === undefined || eff.budget <= 0) return null
    const yuan = session ? costOf(sessionTotals(session), ratesFor(modelOf(session))).yuan : (loadLedger()[sid]?.cost ?? 0)
    if (yuan < eff.budget) return null
    log({ event: 'budget-block', session: sid.slice(0, 8), project, scope: eff.scope, budget: eff.budget, yuan })
    return `\n\n[cost-guard] 预算闸门：${projectName(project)} 已用尽预算（${eff.scope === 'session' ? '会话级' : '项目级'} ¥${eff.budget.toFixed(3)}，已花 ¥${yuan.toFixed(3)}），已停止进一步计费。请用现有上下文信息完成收尾并直接交付，不要再开新的工具调用或探索。如需继续：用 cost_budget 提高预算，或 cost_budget 0 清除预算（回到无硬顶）。`
  }
  /** 合法的短路流（llm-invariant 要求 block-start → delta → block-end → finish）。 */
  function gateChunkStream(text) {
  ...
  try {
    ctx.on('llm/stream', (options, next) => {
      const text = gateDecision(options?.sessionId)
      if (text) return gateChunkStream(text)
      return next()
    }, { global: true, prepend: true })
  } catch (error) {
    log({ event: 'gate-middleware-failed', error: String(error) })
  }

  /** 解析"当前会话/项目"（确定性）：
   *  ① 工具调用上下文 exec.agent（agent-loop 盖章，代表"这个工具是为哪个会话跑的"）——最准；
   *  ② ctx.get('agent')（运行上下文，若曾注入）；
   *  ③ 兜底：最近活动会话（activityAt 随每次 session/event 更新，与面板「当前项目」同源）。
   *  旧版兜底 `agents 表末位`（按插入序取最后捕获的会话）在多个会话并行时会把工具调用
   *  误映射到别的项目（出现过 cost_status/cost_gate 把守卫测试投到新优化思路），已删除。 */
  function currentSession(exec) {
    ...
    const latest = latestId ? agents.get(latestId)?.session : undefined
  ...
  function stateSnapshot(forceSessionId) {
    ...
    return {
      ...
      optabEnabled: (() => { const o = loadOptab(); return latestId ? (o.sessions[latestId] ?? o.enabled) : o.enabled })(),
      config: { ...config },
      now: now.toISOString(),
      peak,
      latest,
    ...
    handler: async (req, res) => {
      ...
      try {
        if (method === 'GET' || method === 'POST') {
          ...
          // 放宽：不再要求会话有 live agent——非活跃会话也能开/关（状态持久化），有 agent 时才注入/撤回引导
          if (path === '/converge' && method === 'POST') {
            ...
            const agent = agents.get(sessionId)
            const project = agent?.session ? projectOfSession(agent.session) : projectOfId(sessionId)
            if (convergedSessions.has(sessionId)) {
              // 关闭：撤回未消费的收敛引导（有 agent 时），退出会话收敛模式
            ...
            // 开：有 agent 注入一次收敛引导；无 agent 也标记收敛（持久化，会话活跃后生效）
            let msgId = null
            if (agent?.session) {
              msgId = injectGuide(agent.session, GUIDE_CONVERGE, true)
              if (msgId !== null) lastConvergeMsg.set(sessionId, msgId)
            }
          ...
          // POST /autodeep {enabled} → 自动成本路由全局开关（持久化 config.json）
          if (path === '/autodeep' && method === 'POST') {
            const body = JSON.parse((await readBody(req)) || '{}')
            const value = Boolean(body.enabled)
            config.autoDeep = { ...(config.autoDeep ?? {}), enabled: value }
            saveConfig(config)
            routedSessionId.clear()
            log({ event: 'ui-autodeep', enabled: value })
            return send({ ok: true, autoDeep: value })
          }
          // POST /optab {enabled, sessionId?} → dsh-optab 托管开关（会话级）：带 sessionId 写该会话覆盖，
          // 不带则写全局默认；optab 插件按当前会话命中 sessions 优先、否则回落全局默认。
          if (path === '/optab' && method === 'POST') {
            const body = JSON.parse((await readBody(req)) || '{}')
            const value = Boolean(body.enabled)
            const sessionId = typeof body.sessionId === 'string' && body.sessionId ? body.sessionId : null
            // 默认关闭保险：UI 只能写会话级；不带 sessionId 的写入一律拒绝，除非显式 scope:"global"（API 保留口）
            if (!sessionId && body.scope !== 'global') return send({ ok: false, error: 'sessionId required (global default locked off)' })
            const saved = saveOptab({ enabled: value, sessionId })
            const o = loadOptab()
            const eff = sessionId ? (o.sessions[sessionId] ?? o.enabled) : o.enabled
            log({ event: 'ui-optab', enabled: value, session: sessionId ? sessionId.slice(0, 8) : null, saved })
            return send({ ok: true, optabEnabled: eff, saved })
          }
  ...
  register({
    ...
    execute(_args, exec) {
      ...
      return [
        ...
        `guard=${guardThresholdInfo(session, eff)}${config.autoDeep?.enabled ? '' : '  (autoDeep 关，防线不触发)'}`,
        `hard-cap=${budget !== undefined ? (scopeOn ? 'ON（预算=硬上限，到顶即停）' : 'set 但守卫未开 → 不生效（cost_enable on 后生效）') : 'off（未设预算，无硬顶）'}  清除/调整预算用 cost_budget`,
      ].join('\n')
    },
  })
