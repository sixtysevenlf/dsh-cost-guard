...
export function apply(ctx) {
  ...
  function checkBudget(session) {
    ...
    const budget = budgetForProject(project)
    if (budget === undefined) return
    const totals = sessionTotals(session)
    const { yuan } = costOf(totals, ratesFor(modelOf(session)))
    if (yuan > budget && !overrunNotified.has(project)) {
      overrunNotified.add(project)
      injectGuide(session, GUIDE_BUDGET)
      log({ event: 'budget-overrun', session: session.id, project, yuan, budget })
    }
  }
  ...
  function stateSnapshot() {
    ...
    for (const s of sessions) {
      ...
      p.sessions.push({ id: s.id, fullId: s.fullId, archived: s.archived })
      p.cost += s.cost ?? 0
    ...
    // 「当前项目」= 最近活动会话所在项目（不是花费最高；新项目花费为 0 也必须能显示自己的开关状态）
    let latestId = null
    let latestAt = -1
    ...
    handler: async (req, res) => {
      ...
      try {
        if (method === 'GET' || method === 'POST') {
          if (path === '/' || path === '/state') {
            return send({ ok: true, ...stateSnapshot() })
          }
          ...
          if (path === '/budget' && method === 'POST') {
            ...
            overrunNotified.delete(project)
            return send({ ok: true, budget: value, project })
          }
          // POST /project/remove {project} → 从统计中彻底删除一个项目：
          // 清除该目录全部会话的 ledger 历史（今日/本周/本月随之减少）、项目级开关/预算配置与缓存。
          if (path === '/project/remove' && method === 'POST') {
            ...
            let removed = 0
            for (const key of Object.keys(ledger)) {
              if (ledger[key].project === project) { delete ledger[key]; removed += 1 }
            }
            saveLedger(ledger)
  ...
  register({
    ...
    execute() {
      ...
      return [
        ...
        `budget=${budget !== undefined ? fmtYuan(budget) : 'unset'}`,
      ].join('\n')
    },
  })
  ...
  register({
    name: 'cost_budget',
    description: '为当前项目设置累计费用预算（¥），超支时自动注入一次「收敛交付」引导。budget=0 清除预算。',
    parameters: { budget: { type: 'number', required: true, description: '预算金额 ¥，0 表示清除' } },
    execute(args) {
      const session = currentSession()
      if (session === undefined) return 'no agent session'
      const project = projectOfSession(session)
      ...
      setProjectBudget(project, value)
      overrunNotified.delete(project)
      if (value === 0) return `project=${projectName(project)}  budget cleared`
      const totals = sessionTotals(session)
      const { yuan } = costOf(totals, ratesFor(modelOf(session)))
      return `project=${projectName(project)}  budget=${fmtYuan(value)}  spent=${fmtYuan(yuan)}  remaining=${fmtYuan(Math.max(0, value - yuan))}`
    },
  })
  ...
  register({
    ...
    execute(args) {
      ...
      const item = addQueued(String(args.task ?? ''), args.maxTokens, project)
      if (!item.task.trim()) return 'queue item ignored: empty task'
