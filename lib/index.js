...
/** dsh-optab 托管开关：读/写 ~/.dsh/cost-guard/optab.json（{enabled:boolean}；缺省关） */
function loadOptab() {
  try { return { enabled: false, ...(existsSync(OPTAB_FILE) ? JSON.parse(readFileSync(OPTAB_FILE, 'utf8')) : {}) } }
  catch { return { enabled: false } }
}
function saveOptab(state) {
  try { writeFileSync(OPTAB_FILE, JSON.stringify({ enabled: Boolean(state.enabled), updatedAt: new Date().toISOString() }, null, 2), 'utf8'); return true } catch { return false }
}
...
export function apply(ctx) {
  ...
  function stateSnapshot(forceSessionId) {
    ...
    return {
      ...
      optabEnabled: loadOptab().enabled,
      config: { ...config },
      now: now.toISOString(),
      peak,
      latest,
    ...
    handler: async (req, res) => {
      ...
      try {
        if (method === 'GET' || method === 'POST') {
          ...
          // POST /optab {enabled} → dsh-optab 插件托管开关（持久化 optab.json；optab 读该文件决定是否生效）
          if (path === '/optab' && method === 'POST') {
            const body = JSON.parse((await readBody(req)) || '{}')
            const value = Boolean(body.enabled)
            const saved = saveOptab({ enabled: value })
            log({ event: 'ui-optab', enabled: value, saved })
            return send({ ok: true, optabEnabled: value, saved })
          }
