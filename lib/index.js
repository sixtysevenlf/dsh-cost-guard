...
export function apply(ctx) {
  ...
  /** force=true 时无视守卫开关（输入框「收敛」按钮 = 用户显式意图，守卫未开也注入）。 */
  function injectGuide(session, text, force = false) {
    if (!force && !enabledForSession(session)) return 'disabled'
    try {
      ...
      try {
        if (method === 'GET' || method === 'POST') {
          ...
          if (path === '/converge' && method === 'POST') {
            ...
            const project = projectOfSession(agent.session)
            const injected = injectGuide(agent.session, GUIDE_CONVERGE, true)
            log({ event: 'converge-button', session: sessionId.slice(0, 8), project, injected })
            return send({ ok: true, injected, sessionId, project })
          }
          if (path === '/' || path === '/state') {
