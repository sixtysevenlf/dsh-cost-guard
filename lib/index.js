        ...
        if (method === 'GET' || method === 'POST') {
          ...
          if (path === '/budget' && method === 'POST') {
            ...
            if (scope === 'session') {
              ...
              return send({ ok: true, budget: value, scope: 'session', sessionId, note: BUDGET_RELATION_NOTE })
            }
            if (scope === 'next') {
              let project = typeof body.project === 'string' && body.project ? body.project : null
              if (!project) {
              ...
              return send({ ok: true, budget: value, scope: 'next', project, note: BUDGET_RELATION_NOTE })
            }
            let project = typeof body.project === 'string' && body.project ? body.project : null
            if (!project) {
              const session = currentSession()
            ...
            return send({ ok: true, budget: value, project, note: BUDGET_RELATION_NOTE })
          }
