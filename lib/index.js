...
export function apply(ctx) {
  ...
  function stateSnapshot() {
    ...
    const latestSessionId = latestId || (latest && latest.sessions && latest.sessions[0] ? latest.sessions[0].fullId : null)
    return {
      enabled: config.enabled,
      sessionEnabled: latest ? latest.enabled : config.enabled,
      sessionOverride: latest ? latest.override : false,
      sessionBudget: latestSessionId ? sessionBudgets.get(latestSessionId) ?? null : null,
      nextBudget: pendingNextBudget && pendingNextBudget.project === latestProjectKey ? pendingNextBudget.budget : null,
      config: { ...config },
      now: now.toISOString(),
      peak,
      latest,
        ...
        if (method === 'GET' || method === 'POST') {
          ...
          if (path === '/budget' && method === 'POST') {
            ...
            if (scope === 'session') {
              ...
              return send({ ok: true, budget: value, scope: 'session', sessionId })
            }
            if (scope === 'next') {
              let project = typeof body.project === 'string' && body.project ? body.project : null
              if (!project) {
              ...
              return send({ ok: true, budget: value, scope: 'next', project })
            }
            let project = typeof body.project === 'string' && body.project ? body.project : null
            if (!project) {
              const session = currentSession()
  ...
  register({
    ...
    execute(args) {
      ...
      if (scope === 'session') {
        ...
        persistSessionBudgets()
        return `session=${session.id.slice(0, 8)}  budget=${value === 0 ? 'cleared' : fmtYuan(value)} (会话级，仅本会话)`
      }
      if (scope === 'next') {
        if (value === 0) pendingNextBudget = null
        else pendingNextBudget = { project, budget: value }
        persistSessionBudgets()
        return `next-session budget=${value === 0 ? 'cleared' : fmtYuan(value)} @ ${projectName(project)}（下一个新会话生效一次）`
      }
      setProjectBudget(project, value)
      overrunNotified.delete('p:' + project)
      const totals = sessionTotals(session)
