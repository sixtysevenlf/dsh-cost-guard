...
export function apply(ctx) {
  ...
  function maybeAutoDeep(session) {
    ...
    // 用户本会话手动点过「收敛」→ 尊重手动选择，不叠加矛盾的深度指令
    if (lastConvergeMsg.has(session.id)) return
    ...
    handler: async (req, res) => {
      ...
      try {
        if (method === 'GET' || method === 'POST') {
          // 输入框「收敛」按钮：对指定会话（默认当前会话）立即注入一次收敛引导（无视守卫开关）
          if (path === '/converge' && method === 'POST') {
            ...
            log({ event: 'converge-button', session: sessionId.slice(0, 8), project, msgId })
            return send({ ok: true, injected: 'ok', convergeMsgId: msgId, sessionId, project })
          }
          // 取消收敛：把刚才注入的收敛引导从 inbox 移除（消息未被模型消费前有效）
          if (path === '/converge/cancel' && method === 'POST') {
            const body = JSON.parse((await readBody(req)) || '{}')
            let sessionId = typeof body.sessionId === 'string' && body.sessionId ? body.sessionId : currentSession()?.id
            if (!sessionId) return send({ ok: false, error: 'no session' })
            const msgId = lastConvergeMsg.get(sessionId)
            if (!msgId) return send({ ok: false, error: 'nothing to cancel' })
            const agent = agents.get(sessionId)
            if (!agent?.session || !agent.inbox || typeof agent.inbox.remove !== 'function') {
              return send({ ok: false, error: 'inbox unavailable' })
            }
            try {
              agent.inbox.remove(msgId)
              lastConvergeMsg.delete(sessionId)
              log({ event: 'converge-cancel', session: sessionId.slice(0, 8), msgId })
              return send({ ok: true, sessionId })
            } catch {
              lastConvergeMsg.delete(sessionId)
              return send({ ok: false, error: 'already consumed (消息已被模型消费，无法取消)' })
            }
          }
          if (path === '/' || path === '/state') {
