...
`;
		...
		function CostGuardPanel() {
			...
			const removeProject = (projectKey) => {
				if (!window.confirm("从统计中删除该项目？\n将清除该目录全部会话的历史数据（今日/本周/本月统计、项目开关与预算一并移除），不可恢复。")) return;
				fetcher("/project/remove", { method: "POST", body: JSON.stringify({ project: projectKey }) }).then((d) => {
					if (d && d.ok) load();
				});
			};
			...
			return h("div", { className: "cg-page" },
				...
				h("div", { className: "cg-card" },
					h("div", { className: "cg-cell" }, h("span", { className: "k" }, "项目分组（各项目独立开关/预算，按花费排序）")),
					!state || !state.projects || !state.projects.length ? h("div", { className: "cg-muted" }, "暂无项目")
						: (state.projects || []).map((x) => h("div", { key: x.key, style: { borderBottom: "1px dashed #262626", padding: "6px 0" } },
							h("div", { className: "cg-row" },
								...
								h("button", { className: "cg-btn cg-btn-danger", onClick: (e) => { e.preventDefault(); e.stopPropagation(); removeProject(x.key); }, title: "从统计中删除该项目（清除该目录全部历史会话数据）" }, "删除")),
							h("div", { className: "cg-row" },
