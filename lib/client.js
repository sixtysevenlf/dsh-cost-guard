						...
						: (state.projects || []).map((x) => h("div", { key: x.key, style: { borderBottom: "1px dashed #262626", padding: "6px 0" } },
							...
							h("div", { className: "cg-sub" },
								(x.sessions || []).slice(0, 20).map((sn) => h("div", { key: sn.fullId, className: "cg-row cg-subrow" },
									h("span", { className: "cg-muted" }, `${sn.id}${sn.archived ? "·归档" : "·活跃"} ${sn.cost != null ? fmtYuan(sn.cost) : "—"} hit ${sn.hitRate != null ? pct(sn.hitRate) : "—"}${sn.tokens ? " " + (((sn.tokens.input ?? 0) + (sn.tokens.cached ?? 0) + (sn.tokens.output ?? 0) + (sn.tokens.reasoning ?? 0)) / 1000 | 0) + "K tok" : ""}`),
