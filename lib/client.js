...
`;
		...
		const HELP_SECTIONS = [
			...
			{ h: "收敛与超支", items: [
				{ t: "开局收敛", d: "点消息输入框右侧的「收敛」按钮，立即对当前会话注入一次收敛引导（按钮变绿后可在消息发出前点「取消」撤回）。" },
				{ t: "收敛指令内容", d: "「[cost-guard] 收敛：用最少的额外步骤完成当前目标——批量读取、复用上下文已有信息、现在就产出。每次工具调用都是计费动作。」" },
				{ t: "自动深度分流", d: "标题栏「深度」开关（默认关）：开启后识别到复杂任务（重构/架构/设计/集成/迁移…）自动注入一次深度引导「值得多花几步做对，避免返工」，每会话一次；不命中关键词或用户已手动收敛时不介入。" },
				{ t: "超支止损", d: "设了预算并开启守卫时，超支会自动提醒收敛；不强制中断。" },
				{ t: "生效条件", d: "预算/超支提醒要项目开关开着才生效；「收敛」按钮和自动深度分流任何时候都能用。" },
			]},
			{ h: "面板怎么看", items: [
...
.cg-convbtn:active{transform:scale(.95)}
.cg-convbtn.ok{color:var(--dsw-alias-success,#4be38a);border-color:currentColor}
.cg-convcancel{color:var(--dsw-alias-label-tertiary,#888);border-color:var(--dsw-alias-border-l2,#444);padding:0 8px}
.cg-convcancel:hover{color:#ff6b6b;border-color:rgba(255,107,107,.5)}
`;
		...
		function ConvergeButton(props) {
			...
			const cancel = () => {
				if (!sessionId) return;
				fetcher("/converge/cancel", { method: "POST", body: JSON.stringify({ sessionId }) }).then((d) => {
					if (d && d.ok) setDone(false);
					else setErr("取消失败: " + JSON.stringify(d));
				});
			};
			return h("span", { className: "cg-convgroup" },
				...
				done ? h("button", { className: "cg-convbtn cg-convcancel", onClick: cancel, title: "取消刚才注入的收敛引导（消息未被消费前有效）" }, "取消") : null);
		}
