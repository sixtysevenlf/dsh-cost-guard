...
`;
		...
		const HELP_SECTIONS = [
			{ h: "这是干嘛的", items: [
				{ t: "作用", d: "统计每个工作目录（项目）的 API 花费；设了预算就是硬上限（花到即停），并用收敛/错峰帮你省钱。" },
			]},
			{ h: "预算怎么设（= 硬上限）", items: [
				{ t: "项目预算", d: "整个目录的所有会话共用一个上限（顶部「预算 ¥」行，项目级默认）。" },
				{ t: "会话预算", d: "只给当前会话设上限，会话行里带「预」标记（工具 cost_budget scope=session）。" },
				{ t: "新会话预算", d: "给该目录下一个新建的会话用一次（项目分组「新会话」栏 / cost_budget scope=next）。" },
				{ t: "生效前提", d: "预算硬顶只在该项目成本守卫开启时生效（「项目」开关 / cost_enable on / 全局默认开）；守卫关 = 预算只统计、不拦截。" },
				{ t: "清除", d: "cost_budget 0 = 清除该层预算 = 回到无硬顶。" },
			]},
			{ h: "收敛与自动路由", items: [
				{ t: "会话级收敛开关", d: "消息输入框右侧「收敛」按钮是开关：点一次开启本会话收敛模式（本会话持续收敛思考），按钮变绿显示「收敛中」；再点一次关闭（撤回未消费引导）。跨会话独立、状态持久。" },
				{ t: "自动成本路由", d: "标题栏「深度」开关（默认关）：开启后，命中复杂度关键词（重构/架构/设计/集成/迁移…且够长）的消息自动选档——预算余量紧张→注入收敛、高峰→注入错峰（可延时重活排队 cost_defer 空闲跑）、空闲且预算充足→注入深度；每会话一次；已开收敛模式或不命中关键词时不介入。" },
				{ t: "动态防线", d: "收敛触发线默认自适应 T=1−2σ/B（σ=上一轮花费、B=预算，随上下文/峰谷自动浮动，夹在 30%~95%）；config.autoDeep.budgetGuardRatio 也可固定成数字。" },
				{ t: "撞线即停", d: "花开到预算由 llm/stream 短路拦截、不再产生任何新计费（在途这一发会走完）；到达前由动态防线先收敛收尾 = 分层兜底。" },
			]},
			{ h: "优化（dsh-optab · 输入框「优化」按钮）", items: [
				{ t: "作用", d: "开启后对每次请求做三层省 token：① 前缀瘦身（滤零调用工具 schema，实测每请求少≈3-4k token）；② 大工具结果双层压缩（确定性头尾预览 + 落库替换，全文存 artifact、跨重启回放）；③ 推理档位自动（关键词/滚动→max，常规→high）。全量语料实测省≈9.4%，叠加档位 high ≈15–18%。" },
				{ t: "开关键", d: "输入框右侧「优化」按钮：点开=生效（按钮变绿显示「优化中」），再点=停用；状态持久化在 ~/.dsh/cost-guard/optab.json，默认关、不影响会话。" },
				{ t: "与收敛配合", d: "同一会话点「收敛」→ 该会话自动进入 optab 收紧模式（档位强制 high、预览/阈值更短），且只影响该会话、不误伤别的会话；重任务实测「收敛+优化」同开约省 87%。" },
				{ t: "峰时/超支", d: "高峰时段自动把 max 封顶为 high（省 2× 峰价）；成本守卫超支信号命中 → 强制收紧（tight）。" },
			]},
			{ h: "面板怎么看", items: [
				{ t: "项目分组", d: "一个目录一行，展开是每个会话的花费明细；带「预」= 该会话有独立预算。" },
				{ t: "删除", d: "点两次确认，把项目/会话从统计里清掉。" },
			]},
			{ h: "错峰队列", items: [
				{ t: "cost_defer", d: "高峰时段（09-12点、14-18点）单价翻倍，任务先排队，空闲了自动跑。" },
				{ t: "cost_peak / cost_status", d: "cost_peak 看峰谷与切换点；cost_status 看已花/预算/防线阈值/hard-cap 状态。" },
			]},
		];
		...
		// 收敛按钮样式（ConvergeButton 是独立插槽组件，必须自带 <style> 才会生效）
		const CONV_STYLES = `
.cg-convgroup{display:inline-flex;align-items:center;gap:4px}
.cg-convbtn{display:inline-flex;align-items:center;gap:5px;height:28px;padding:0 10px;color:var(--dsw-alias-label-secondary,#aaa);background:transparent;border:1px solid var(--dsw-alias-border-l1,#333);border-radius:999px;font-size:12px;cursor:pointer;white-space:nowrap;transition:all .15s;line-height:1;font-weight:400}
.cg-convbtn:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(128,128,128,.14));color:var(--dsw-alias-label-primary,#e8e8e8);border-color:var(--dsw-alias-border-l2,#444)}
.cg-convbtn:active{transform:scale(.95)}
.cg-convbtn.ok{background:#22c55e;border-color:#22c55e;color:#06210f;font-weight:700}
.cg-convbtn.ok:hover{background:#16a34a;border-color:#16a34a;color:#06210f}
.cg-convbtn.busy{opacity:.6;cursor:wait}
`;

		// 消息输入框右侧「收敛」按钮 = 会话级收敛开关（pill 外形模仿输入框工具按钮）：
		// 点击开启 → 本会话进入收敛模式（持久）；再点击关闭 → 撤回未消费引导并退出。
		// 开启态 = 实心绿底高亮（一眼可辨）；切换会话/刷新时从服务端同步真实状态。
		function ConvergeButton(props) {
			...
			const sessionId = props && props.sessionId ? props.sessionId
				: (props && props.session ? props.session.id : undefined)
				?? (useSessions ? useSessions((s) => s.current) : undefined);
			const [on, setOn] = useState(false);
			const [err, setErr] = useState(null);
			const [busy, setBusy] = useState(false);
			const sync = () => {
				if (!sessionId) return;
				fetch(API + "/state?session=" + encodeURIComponent(sessionId), { headers: { "content-type": "application/json" } })
					.then((r) => r.json())
					.then((d) => { if (d && d.ok && d.sessionConverged !== undefined) setOn(Boolean(d.sessionConverged)); })
					.catch(() => {});
			};
			React.useEffect(() => { setErr(null); sync(); }, [sessionId]);
			const hit = () => {
				...
				fetcher("/converge", { method: "POST", body: JSON.stringify({ sessionId }) }).then((d) => {
					setBusy(false);
					if (d && d.ok) {
						if (typeof d.converged === "boolean") setOn(d.converged);
						else { setOn((o) => !o); setErr("需重启 web 后开关才真正生效"); }
					} else setErr("失败: " + JSON.stringify(d));
				});
			};
			return h("span", { className: "cg-convgroup" },
				h("style", null, CONV_STYLES),
				h("button", {
					className: "cg-convbtn" + (on ? " ok" : "") + (busy ? " busy" : ""),
					onClick: hit,
					disabled: busy,
					title: err ? err : (on ? "本会话处于收敛模式——再点一次关闭" : "开启本会话的收敛模式：持续少探索、直接交付"),
					style: err && !on ? { borderColor: "#ff6b6b", color: "#ff6b6b" } : undefined,
				},
		...
		// 输入框右侧「优化」按钮 = dsh-optab 插件托管开关（本插件已整合进成本守卫）：
		// 点开 → optab 生效（绿=优化中）；再点 → 停用（optab 读到 optab.json.enabled=false 后不再动作）。
		function OptabButton(props) {
			const useSessions = props && props.useSessions;
			const sessionId = props && props.sessionId ? props.sessionId
				: (props && props.session ? props.session.id : undefined)
				?? (useSessions ? useSessions((s) => s.current) : undefined);
			...
			const sync = () => {
				fetch(API + "/state?session=" + encodeURIComponent(sessionId || ""), { headers: { "content-type": "application/json" } })
					.then((r) => r.json())
					.then((d) => { if (d && d.ok && d.optabEnabled !== undefined) setOn(Boolean(d.optabEnabled)); })
					.catch(() => {});
			};
			React.useEffect(() => { setErr(null); sync(); const timer = window.setInterval(sync, 8000); return () => window.clearInterval(timer); }, [sessionId]);
			const hit = () => {
				if (!sessionId) { setErr("无当前会话"); return; }
				setErr(null); setBusy(true);
				fetcher("/optab", { method: "POST", body: JSON.stringify({ enabled: !on, sessionId }) }).then((d) => {
					setBusy(false);
					if (d && d.ok) { if (typeof d.optabEnabled === "boolean") setOn(d.optabEnabled); else setOn((o) => !o); }
					else setErr("失败: " + JSON.stringify(d));
					sync(); // 点击后再同步一次，杜绝显示与真实状态脱节
				});
			};
			return h("span", { className: "cg-convgroup" },
				h("style", null, CONV_STYLES.replace(/\.cg-convbtn/g, ".cg-optbtn").replace(/\.cg-convgroup/g, ".cg-convgroup")),
				h("button", {
					className: "cg-optbtn" + (on ? " ok" : "") + (busy ? " busy" : ""),
					onClick: hit,
					disabled: busy,
					title: err ? err : (on ? "本会话已启用 dsh-optab——再点一次停用" : "启用本会话的 dsh-optab：前缀瘦身 + 工具输出压缩 + 档位自动"),
					style: err ? { borderColor: "#ff6b6b", color: "#ff6b6b" } : undefined,
				},
					h("svg", { viewBox: "0 0 16 16", width: 14, height: 14, fill: "none", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" },
						h("path", { d: "M4 4 12 12M12 4 4 12M8 2 9.5 6.5 14 8 9.5 9.5 8 14 6.5 9.5 2 8 6.5 6.5Z" })),
					on ? "优化中" : "优化"));
		}
		...
		function CostGuardPanel(props) {
			...
			const autoDeepOn = state ? Boolean(state.autoDeep) : false;
			const toggleAutoDeep = () => {
				const next = !autoDeepOn;
				fetcher("/autodeep", { method: "POST", body: JSON.stringify({ enabled: next }) }).then((d) => {
					if (d && d.ok) load();
				});
			};
			...
			return h("div", { className: "cg-page" },
				...
				h("div", { className: "cg-h" },
					...
					h("label", { className: "cg-toggle" + (autoDeepOn ? " on" : ""), onClick: toggleAutoDeep, title: "自动成本路由（原「自动深度分流」）：复杂任务按 预算余量×峰谷 自动选 深度/收敛/错峰——预算紧张→收敛防超支、高峰→错峰排队、空闲+充足→深度；已开收敛模式时不介入" },
						h("span", { className: "cg-switch" }),
						h("span", { className: "cg-muted" }, "深度:" + (autoDeepOn ? "开" : "关"))),
		...
		function apply(ctx) {
			...
			ctx.effect(() => ctx.slots.inject("conversation.input.right", () => ctx.slots.register({
				name: "conversation.input.right", id: "cost-guard-optab", order: 82, label: "优化",
			}, OptabButton)), "cost-guard: optab button");
		}
		//#endregion
		exports.apply = apply;
