...
`;
		...
		const HELP_SECTIONS = [
			...
			{ h: "收敛与超支", items: [
				{ t: "会话级收敛开关", d: "消息输入框右侧「收敛」按钮是开关：点一次开启本会话收敛模式（注入一次收敛引导，此后该会话持续收敛思考），按钮变绿显示「收敛中」；再点一次关闭（撤回未消费引导）。跨会话独立、状态持久。" },
				{ t: "收敛指令内容", d: "「[cost-guard] 收敛：用最少的额外步骤完成当前目标——批量读取、复用上下文已有信息、现在就产出。每次工具调用都是计费动作。」" },
				{ t: "自动深度分流", d: "标题栏「深度」开关（默认关）：开启后识别到复杂任务（重构/架构/设计/集成/迁移…）自动注入一次深度引导「值得多花几步做对，避免返工」，每会话一次；不命中关键词、或该会话已开启收敛模式时不介入。" },
				{ t: "超支止损", d: "设了预算并开启守卫时，超支会自动提醒收敛；不强制中断。" },
				{ t: "生效条件", d: "预算/超支提醒要项目开关开着才生效；「收敛」开关和自动深度分流任何时候都能用。" },
			]},
			{ h: "面板怎么看", items: [
		...
		function CostGuardPanel(props) {
			...
			return h("div", { className: "cg-page" },
				...
				h("div", { className: "cg-h" },
					...
					h("label", { className: "cg-toggle" + (autoDeepOn ? " on" : ""), onClick: toggleAutoDeep, title: "自动深度分流：开启后，识别到复杂任务（重构/架构/设计/集成…）自动注入深度引导，避免返工" },
						h("span", { className: "cg-switch" }),
