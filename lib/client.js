...
.cg-convbtn{background:rgba(74,158,255,.12);color:var(--theme-accent,#4a9eff);border:1px solid rgba(74,158,255,.4);border-radius:6px;padding:2px 10px;font-size:12px;cursor:pointer;height:24px;white-space:nowrap;line-height:1.4}
.cg-convbtn:hover{background:rgba(74,158,255,.28)}
.cg-convbtn.ok{background:rgba(46,204,113,.2);color:#4be38a;border-color:rgba(46,204,113,.5)}
...
`;
		...
		// 消息输入框右侧「收敛」按钮：点击即对当前会话注入一次收敛引导
		function ConvergeButton(props) {
			...
			return h("button", {
				...
				style: err ? { borderColor: "#ff6b6b" } : undefined,
			}, done ? "已收敛 ✓" : "收敛");
		}
