...
`;
		...
		function CostGuardPanel() {
			...
			const [nextBudgetDraft, setNextBudgetDraft] = useState("");
			...
			const applyNextBudget = () => {
				const v = parseFloat(nextBudgetDraft);
				if (Number.isNaN(v) || !currentKey) return;
				fetcher("/budget", { method: "POST", body: JSON.stringify({ budget: v, scope: "next", project: currentKey }) }).then((d) => {
					if (d && d.ok) { setNextBudgetDraft(""); load(); }
				});
			};
			...
			return h("div", { className: "cg-page" },
				...
				h("div", { className: "cg-card" },
					...
					h("div", { className: "cg-grid" },
						...
						h(Cell, { k: "预算", v: (state && state.sessionBudget != null ? "会话 " + fmtYuan(state.sessionBudget) + (s && s.budget != null ? " / 项目 " + fmtYuan(s.budget) : "") : (s && s.budget != null ? fmtYuan(s.budget) : "未设")) }),
					),
					...
					h("div", { className: "cg-row" },
						h("span", { className: "cg-muted" }, "新会话预算 ¥："),
						h("input", { className: "cg-input", value: nextBudgetDraft, placeholder: state && state.nextBudget != null ? "当前 " + fmtYuan(state.nextBudget) + "；0=清除" : "下一个新会话；0=清除",
							onChange: (e) => setNextBudgetDraft(e.target.value), onKeyDown: (e) => { if (e.key === "Enter") applyNextBudget(); } }),
						h("button", { className: "cg-btn", onClick: () => applyNextBudget() }, "设置"),
					),
