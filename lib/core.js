...
/** 高峰时段（本地时区）：每日 09:00–12:00 与 14:00–18:00。 */
...
export function nextWindow(date = new Date()) {
  ...
  if (nowPeak) {
    // 高峰中：找当前时段结束（09:00-12:00 → 12:00；14:00-18:00 → 18:00）
    const h = d.getHours()
    const end = h < 12 ? 12 : 18
    d.setHours(end, 0, 0, 0)
    return { now: 'peak', next: d, nextIs: 'offPeak' }
  }
  // 空闲中：找下一个高峰开始（今日 09:00 / 14:00，已过则次日）
...
export const GUIDE_BUDGET_TIGHT =
  '\n\n[cost-guard] 预算接近上限：收敛——以最少的额外步骤完成当前目标（批量读取、复用已有信息、现在就交付），防止超支。'
