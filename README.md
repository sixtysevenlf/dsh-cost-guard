...
- **预算输入框（三级作用域）**：① **项目级**（默认）——该目录全部会话共享；② **会话级**——只限当前会话（`cost_budget` 加 `scope=session`）；③ **新会话预算**——预置给该目录**下一个新建的顶层会话**（一次性消费，`scope=next`；子代理不会消费）。卡片「预算」格同时显示会话级/项目级；会话明细行带「预 ¥X」标记的表示该会话有自己的会话级预算。超支自动注入收敛引导，持久化在 `~/.dsh/cost-guard/session-budgets.json`。
...
- 可分发的安装包：GitHub Release 附件 `dsh-external-dsh-cost-guard-0.1.4.tgz`（npm pack 产物，`dsh plugin --profile web add <tgz>` 安装）
