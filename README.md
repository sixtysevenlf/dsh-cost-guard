...
开启后，识别到**复杂任务**（含"重构/架构/设计/集成/迁移/分布式/性能/复杂…"等关键词且消息足够长）
自动向该会话注入一次深度引导（"值得多花几步做对，避免返工=重复计费"），每会话一次；
不命中关键词、消息过短、或该会话已开启收敛模式时不介入（防止收敛与深度指令矛盾）。
...
- 可分发的安装包：GitHub Release 附件 `dsh-external-dsh-cost-guard-0.1.20.tgz`（npm pack 产物，`dsh plugin --profile web add <tgz>` 安装）
